...
import { catchError, combineLatest, forkJoin, map, mergeMap, of, switchMap, zip } from "rxjs";
    ...
    return actions$.pipe(
        ...
        switchMap(() => {
            return vehiclesData.getVehicles().pipe(
                mergeMap(vehicles => {
                    return zip(vehicles.map(vehicle => vehiclesData.getVehicleData(vehicle.id).pipe(
